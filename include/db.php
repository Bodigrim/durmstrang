...
function db_and_errors_init($servers){
	...
	foreach($servers as $id=>$server){
		list($dbhost,$dbuser,$dbpass,$dbname,$debug,$onair)=$server;
...
function fetch_rows1($sql, $depth=0, $unique=false){
	return fetch_rows(query($sql), $depth, $unique);

	$hash = md5($sql);
	$key = "selina_sql_$hash";

	$ret = @unserialize(gRedis($key));
	if($ret)
		return $ret;

	$result = query($sql);
	$ret = fetch_rows($result, $depth, $unique);

	sRedis($key, serialize($ret));
	expireRedis($key, 1000);
...
function fetch_mapping1($sql){
	return fetch_rows(query($sql), 1, true);
	}
