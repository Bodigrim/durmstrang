...
class GetVarClass{
	...
	private function add0($str,$upto){
		while(strlen($str)<$upto)
			$str="0$str";
		return $str;
		}
	...
	private function valid_iso_date($date,$default){
		...
		$year=self::add0($year,2);
		if(strlen($year)==2)
			$year="20$year";
		$year=self::add0($year,4);

		$month=self::add0($month,2);
		$day=self::add0($day,2);
	...
	function getkeyword($name,$default=FALSE){
		return trim(preg_replace("/\s+/u", " ", preg_replace("/[^-\w\d\s]+/u", " ", strip_tags($this->getvar($name,$default)))));
		}
